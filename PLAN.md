...
| P2-1.c | 实现阶段第一个 PR（基础设施落地）：把 ADR-009 / ADR-010 / ADR-011 三件套从 trait 签名 + 状态机 + 单测清单 落到 src-tauri/src/peer / app/lifecycle / crypto 三个 module 的实际 Rust 代码 + ≥ 18 条单测 | `DONE (3/3 REVIEW_PASSED_WITH_NITS)` | backend-implementer | **🎯 三件套全 REVIEW_PASSED**（2026-05-09）：PR-1 crypto APPROVED 0 必修 / PR-2 PeerRegistry 2 补丁已落 / PR-3 Lifecycle 2 [低/nit] reviewer 建议挂 PR-4 第一个 commit 顺手清理（dead code 删 + ADR-010 #9 单测注释）。累计 43/43 单测全过（crypto 18 + peer 14 + app 11）。8 必修条目 MUST-1~5 项目层全部代码层闭环（MUST-6/7/8 feature 层留 PR-4+）。**6 commits 闭环**：a23f086 / 3acafb1 / b3382cb / 6c6ae7d / 69597a4 / 1b7e035 / 25fe411。基础设施层完成，PR-4 HTTP server / 剪切板 / 心跳 / Tauri 命令解锁 |
