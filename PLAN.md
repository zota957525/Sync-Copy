...
| P2-1.e | PR-5 后端 MVP 端对端：handler 真业务（handshake ECDH / clipboard 加解密 / heartbeat / leave 原子 remove）+ network/client.rs（broadcast_clipboard / broadcast_leave / dial_handshake）+ lifecycle step 3 leave 广播接入 + AppState clipboard_apply_tx 占位 | `BLOCKED_BY_REVIEW` | backend-implementer | **PR-5 IMPL_DONE 表面 83/83 单测过；review BLOCKED**（2026-05-09 commit f56bc68）：code-reviewer 发现 3 个 ADR 契约级严重违反（review 段在 specs/clipboard-text-sync.md 第 8 节）：① **MUST-4 违反**：PeerRegistry.remove/ban 未调 client_pool.remove（leave handler 注释虽写"原子"实际没调，违反 ADR-009 第 3.5 节）；② handshake.rs:67-74 自连校验跳过（TODO 占位）；③ handshake.rs:168-173 `device_id: "placeholder-my-device-id"` 占位字符串（N=2 happy path 能跑因占位串相同索引到同项，N=3+ 必崩）。**v5-2 第 1 类硬关卡触发首例**：必须停下上报用户拍板（A 全修 + 回溯 PR-2 / B 修 #2#3 留 #1 PR-6 / C ADR supersede）|
