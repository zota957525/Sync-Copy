...
| P2-1.c | 实现阶段第一个 PR（基础设施落地）：把 ADR-009 / ADR-010 / ADR-011 三件套从 trait 签名 + 状态机 + 单测清单 落到 src-tauri/src/peer / app/lifecycle / crypto 三个 module 的实际 Rust 代码 + ≥ 18 条单测（PeerRegistry 7 + Lifecycle 5 + crypto 6） | `IN_PROGRESS (1/3 REVIEW_PASSED, 2/3 IMPL_DONE)` | backend-implementer | **PR-1 REVIEW_PASSED**（commit b3382cb / 18 单测 / APPROVED 0 必修）。**PR-2 PeerRegistry IMPL_DONE（2026-05-09）**：src-tauri/src/peer/{mod,rate_limit}.rs ~565 行 + 13 单测全过（累计 31/31）。MUST-2/4/5 闭环；ADR-009 v1.2 P1/P2/P3/P4 4 补丁全落地。**v2-9 遵守**：未改 PLAN.md（PR-1 越界教训生效）。等 code-reviewer review PR-2，通过后派 PR-3 Lifecycle |
