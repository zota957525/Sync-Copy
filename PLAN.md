...
| P2-1.c | 实现阶段第一个 PR（基础设施落地）：把 ADR-009 / ADR-010 / ADR-011 三件套从 trait 签名 + 状态机 + 单测清单 落到 src-tauri/src/peer / app/lifecycle / crypto 三个 module 的实际 Rust 代码 + ≥ 18 条单测 | `DONE (3/3 REVIEW_PASSED_WITH_NITS)` | backend-implementer | **🎯 三件套全 REVIEW_PASSED**（2026-05-09）：PR-1 crypto APPROVED 0 必修 / PR-2 PeerRegistry 2 补丁已落 / PR-3 Lifecycle 2 [低/nit] 已挂 PR-4 顺手清理。累计 43/43 单测（crypto 18 + peer 14 + app 11）。8 必修 MUST-1~5 项目层闭环。8 commits 闭环（a23f086 ~ 225ddda）|
| P2-1.d | HTTP server skeleton + 4 feature 层必修（MUST-3/6/7/8）+ sanitize 模块 + lifecycle step 5 真落 axum bind | `REVIEW_PASSED` | backend-implementer | **PR-4 REVIEW_PASSED（2026-05-09）**：commit 937fdda + 25 新单测累计 68/68 + MUST-3/6/7/8 全闭环。code-reviewer **APPROVED 0 必修**（5 聚焦点全 PASS；4 信息项 + 低 nit 全留 PR-5+ 处理：① bind 失败真 unwind 时序 ② handshake handler 接入 PeerState 时去 _sanitized_name 占位 ③ peers_announce 引入 DTO 时补三件套 ④ ADR-010 #9 单测注释保留）。9 commits 闭环（a23f086~937fdda）。8 必修 MUST-1~8 全部代码层闭环。**handler 真业务（crypto 真加解密 / 剪切板 arboard / 心跳 worker / leave broadcaster）留 PR-5+** |
| P2-1.e | PR-5 后端 MVP 端对端：handler 真业务（handshake ECDH / clipboard 加解密 / heartbeat / leave 原子 remove）+ network/client.rs（broadcast_clipboard / broadcast_leave / dial_handshake）+ lifecycle step 3 leave 广播接入 + AppState clipboard_apply_tx 占位 | `REVIEW_PASSED` | backend-implementer | **🎯 backend MVP 里程碑达成（2026-05-10）**：PR-5 BLOCKED → PR-5b 全修 → **APPROVED 0 必修**（review 段 specs/clipboard-text-sync.md 第 8.7 节）。3 严重违反全闭环：MUST-4 PeerRegistry 内嵌 client_pool / handshake 自连 403 / my_device_id uuid。87/87 单测全过。新发现 4 条 [低] nit（Default for PeerRegistry / leave 测试遗留 / `_ => "text"` 兜底 / banned 后置）挂 PR-6 顺手清理。13 commits 闭环（a23f086~ef2979a）|
| P2-1.f | PR-6a 真业务接入第一批：arboard 1s 轮询线程（std::thread）+ mpsc 真接 + lifecycle step 4 集成 + 4 [低] nit 清理 | `REVIEW_PASSED (PR-6a / PR-6a')` | backend-implementer | **PR-6a IMPL_DONE**（commit fd0573c）→ review CHANGES_REQUESTED 4 补丁（1 严重 ADR-010 第 3.3 节 100ms 软上限未真实现 + 1 中 broadcast_rx warn 噪音 + 2 低 doc/单测覆盖）→ **PR-6a' 静默修**（commit 994e16a，按新策略主窗口编排不上报用户）→ review v4 **APPROVED 0 必修**：100ms 真实现（done_tx/done_rx + recv_timeout + Timeout detach + warn）release 实测 0.01s ≪ 100ms / noise trace 降级 / doc-comment 反映真业务 / AC #6 invalid AAD 拒 + AC #7 retry simulate。99/99 单测全过（96 + 3）。2 commits（fd0573c + 994e16a）|
| P2-1.g | PR-6b 真业务接入第二批：heartbeat worker（5s 主动 ping）+ 隐形掉线检测（v0 实战 bug 修复）+ force_rebuild 6 步（Shutting/banned 校验 / replace / 窗口期重检 / dial_handshake / reset）+ PeerRegistry 6 新方法 + ping 真实现 | `REVIEW_PASSED` | backend-implementer | **PR-6b IMPL_DONE → APPROVED 0 严重 0 中**（commit a8a3a08，review 段 specs/peer-heartbeat.md 第 8.1 节）。**6 ADR 必修条目全闭环**：① ADR-010 第 3.6 节 Shutting 禁 replace；② ADR-008 第 5.3 节 banned 校验；③ ADR-008 卡 7 must-fix #1 last_successful_sync_at 仅 broadcast 写（grep + 单测双重证明）；④ ADR-008 MUST-2 zeroize auto-zero；⑤ ADR-009 第 4.3 节副作用 #3 窗口期重检；⑥ 隐形掉线语义。9 新单测累计 108/108 全过。5 [低] nit 挂 PR-7 扫尾 |
| P5-1 | qa-tester 集成测试 + 跨平台 CI（macOS + Windows）+ 手测 checklist | `TEST_PASSED` | qa-tester | **P5-1 IMPL_DONE**（commit 5a0ed3c）：8 集成测试 + CI gate job + 9 手测场景。**cargo test 116 passed**。spec gap 发现 → 用户回 A 补 gossip |
| P5-2 | qa-tester 补 N=3 gossip 真集成测试 + 手测 S2 PASS | `TEST_PASSED` | qa-tester | **P5-2 IMPL_DONE**（commit 085ae08）：test_three_instance_gossip_mesh（104 行 + 5s polling timeout）验证 gossip 双路径（C 通过 resp.peers 拉 A + A 通过 broadcast_announce 拉 C）；6 断言全 mapping AC #2 + ADR-009 invariant 3。**cargo test 123 passed**（114 lib + 9 tests）。**🎯 v2.0 backend 范围 100% 完成** — 19 commits 闭环 |
| P5-3 | qa-tester 复跑 history-list AC #1/#2 + floating-ball 8 AC（PR-7 闭环验证）| `TEST_PASSED` | qa-tester | **P5-3 IMPL_DONE**（commit ad78541）：2 新集成测试（test_local_clipboard_change_pushes_history + test_remote_clipboard_ingest_pushes_history）+ tests/pr-fe-3-history-ball.md S10-S21 12 手测场景 + 18 AC 全 mapping 矩阵。**cargo test 153 passed**（142 lib + 11 tests）+ 4 处 history-updated emit grep 证据 |
| P5-4 | docs-writer 重写 v2.0 全部用户/开发者文档 | `DOCS_DONE` | docs-writer | **P5-4 IMPL_DONE**（commit 41c9104）：使用说明.md 230 行 + 项目架构.md 454 行（11 节稳定结构，每节引用 ADR/spec 精确章节）+ CHANGELOG.md 93 行（Keep a Changelog v1.1.0 [0.2.0] 含 Added 13/Changed 4/Fixed 2/Security 8/Documentation 3）+ README.md 62 行。0 § / 0 占位 / v0 banner 已移除 |
| P5-5 | release-engineer 升 v0.2.0 + RELEASE_NOTES + CI 验证 | `CI_GREEN_ARTIFACT_UPLOADED` | release-engineer | v0.2.0 release 全链路 **🎉 全绿**（gh run 25917166153，2026-05-15）：version 三处同步 / RELEASE_NOTES_v0.2.0.md 127 行 / CI gate + matrix 双绿 + Build artifact 上传成功。**Windows CI 修复历经 7 次迭代**：PR #2/3/4/5/6 失败教训 → PR #7 策略 B（Mac 跑单测 / Windows 跑 cargo build）真根因解决（windows-2025 runner + webview2-com 0.38.2 DLL 入口点错位）。**artifact**：sync-copy-macos-universal-v0.2.0 + sync-copy-windows-x64-v0.2.0（14 天保留期）。**用户实测发现 3 bug** → 进 v0.2.1 修复（见 P5-6） |
| P5-6 | v0.2.1 patch release：v0.2.0 实测 bug 修复版 | `READY_FOR_PR` | backend-impl + frontend-impl + release-engineer + qa-tester 联动 | **v0.2.0 用户实测 5 bug 全闭环**（2026-05-15 ~ 2026-05-16）：① macOS Gatekeeper 拒绝（CI ad-hoc codesign + 使用说明.md FAQ Q6）/ ② 端口 5858 静默失败（lifecycle step 5 bind 同步前置 + show_startup_error_dialog + exit(1)）/ ③ 加入按钮无 onclick + JoinDialog 缺失（btn-join 占位 → 真接 + 新建 JoinDialog 359 行 + FloatingWindow 加 'join' view）/ ④ IPC camelCase 系统性错位（ipc.ts 3 处 join/approve/reject 改 snake_case）/ ⑤ B3 abort→exit(1) 消除 macOS"意外退出"噪音 + B4 fatal error.log 完整 v4-7 三件套。**release-engineer 准备**：version 三处 0.2.0→0.2.1 / CHANGELOG 119 行新 [0.2.1] 段 / RELEASE_NOTES_v0.2.1.md 85 行 / 12 文件 working tree。**qa-tester 2026-05-16 本机 dev mode 实测全 PASS**（端口监听 + HTTP 422 + JoinDialog 链路 + B3 无新 crash report + B4 error.log 真生成）。**等 git 同事开 PR #9** |
| P2-1.h | PR-7 group-discovery AC #2 gossip mesh auto-expansion：HandshakeResp 加 peers / /peers/announce 端点 / 客户端 gossip 循环（dial peers in resp + broadcast announce）+ GOSSIP_MAX_CONCURRENT=3 防 cascade + 失败不写 registry 防 zombie | `REVIEW_PASSED` | backend-implementer | 用户回"采纳推荐"= A 补 gossip → **PR-7 IMPL_DONE + APPROVED 0 必修**（commit bacb9d2 / review 段 specs/group-discovery.md 第 8 节）。**4 聚焦点全 PASS**：协议层 PeerStub 最小化 + serde 兼容 / handshake peers 过滤 Approved + 排除 req / announce 鉴权全闭环（origin approved + self/banned/dedupe + 通用 403）/ 客户端 cascade 双闸（并发 ≤ 3 + 一跳终止）+ 0 zombie。7 新单测累计 114 lib + 8 tests = 122 passed。**3 [低] nit**：nit #1 peers.rs:59-63 注释撒谎说"RateLimiter 限流"实未调（v5-1 同类自查问题）/ nit #2 双层 Arc 冗余 / nit #3 GossipAnnouncePayload.seq 字段未消费。挂 PR-7a 修 |
