...
| P5-1 | qa-tester 集成测试 + 跨平台 CI（macOS + Windows）+ 手测 checklist | `TEST_PASSED` | qa-tester | **P5-1 IMPL_DONE**（commit 5a0ed3c）：8 集成测试全过（双实例 handshake + AES-GCM / 自连拒 / leave 原子 / 篡改 AAD 拒 / unknown peer 拒 / DTO serde / crypto round-trip + 篡改 / device_id 占位回归）；CI gate job（macos-latest + windows-latest matrix，cargo test --lib/--tests + clippy --all-targets -D warnings）；手测 9 场景。**cargo test 116 passed**（108 lib + 8 tests）。**发现 1 项 spec gap**：group-discovery AC #2 gossip 自动扩展未实现（backend 未做 /peers 端点的拉取逻辑），影响 N≥3 设备组。**需用户决策**：补 PR-7 gossip / 推迟 v2.1 / v2.0 known limitation |
