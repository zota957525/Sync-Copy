...
| P2-1.d | HTTP server skeleton + 4 feature 层必修（MUST-3/6/7/8）+ sanitize 模块 + lifecycle step 5 真落 axum bind | `IMPL_DONE` | backend-implementer | **PR-4 IMPL_DONE（2026-05-09）**：src-tauri/src/network/{mod,error,protocol,handlers/*}.rs + peer/sanitize.rs ~1300 行 + lifecycle.rs step 5 真起 `axum::serve.with_graceful_shutdown` + lib.rs 注册 network module + PR-3 2 nit 清理。**25 新单测累计 68/68 全过**（network::error 6 / file 3 / router smoke 1 / peer::sanitize 16 + 13 nit）。MUST-3 (409→403 通用 body) / MUST-6 (/file seq dedupe + size 双校验 7MB DefaultBodyLimit) / MUST-7 (handshake DoS 限流 3/10/60s) / MUST-8 (sanitize 三函数 device_name/filename/log_field) 全闭环。**v2-9 + 诚实自检全守**。等 code-reviewer review PR-4。**handler 真业务（crypto 真加解密 / 剪切板 / 心跳 / leave 实际逻辑）留 PR-5+** |
