...
| P2-1.c | 实现阶段第一个 PR（基础设施落地）：把 ADR-009 / ADR-010 / ADR-011 三件套从 trait 签名 + 状态机 + 单测清单 落到 src-tauri/src/peer / app/lifecycle / crypto 三个 module 的实际 Rust 代码 + ≥ 18 条单测（PeerRegistry 7 + Lifecycle 5 + crypto 6） | `IN_PROGRESS (1/3+2/3 REVIEW_PASSED, 3/3 IMPL_DONE)` | backend-implementer | **PR-1 + PR-2 REVIEW_PASSED**（commits b3382cb/6c6ae7d/69597a4/1b7e035）。**PR-3 Lifecycle IMPL_DONE（2026-05-09）**：src-tauri/src/app/{mod,lifecycle,client_pool,state}.rs ~822 行 + lib.rs 重写 + Cargo.toml 加 3 依赖（tokio-util/tracing-appender/thiserror）+ 11 新单测（累计 43/43 全过）。MUST-5 panic hook 落地 + ADR-009 client_pool 接口契约 + ADR-010 v1.2 4 补丁全落实 + 占位空 worker（heartbeat/health/leave 实际业务留 PR-4+）。**v2-9 + 诚实自检全守**：未改 PLAN.md，cargo test --list + git status -s 真实粘贴。等 code-reviewer review PR-3 |
