...
| P2-1.c | 实现阶段第一个 PR（基础设施落地）：把 ADR-009 / ADR-010 / ADR-011 三件套从 trait 签名 + 状态机 + 单测清单 落到 src-tauri/src/peer / app/lifecycle / crypto 三个 module 的实际 Rust 代码 + ≥ 18 条单测（PeerRegistry 7 + Lifecycle 5 + crypto 6） | `BACKLOG` | backend-implementer | 前置：P2-1.b 第一批 ACCEPTED。**MUST 1-5**（ADR-008 项目层）必须落地：MUST-1 AAD 绑值 / MUST-2 zeroize / MUST-3 状态码 409→403 / MUST-4 PeerRegistry.remove 原子顺序 / MUST-5 panic message 不含变量插值。等用户拍板"是否启动 P2-1.c"|
