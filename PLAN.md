...
| P2-1.c | 实现阶段第一个 PR（基础设施落地）：把 ADR-009 / ADR-010 / ADR-011 三件套从 trait 签名 + 状态机 + 单测清单 落到 src-tauri/src/peer / app/lifecycle / crypto 三个 module 的实际 Rust 代码 + ≥ 18 条单测（PeerRegistry 7 + Lifecycle 5 + crypto 6） | `IN_PROGRESS (1/3 PR-1 IMPL_DONE waiting review)` | backend-implementer | **PR-1（crypto module）IMPL_DONE（2026-05-09）**：ADR-011 落地 src-tauri/src/crypto/{mod,x25519,aes_gcm}.rs 共 ~660 行（含注释/单测）+ Cargo.toml 加 zeroize 1.8。**18 条单测全通过**（mod 3 / aes_gcm 10 / x25519 5；ADR-011 第 3.6 节必须 6 条全覆盖 + 12 条额外加固）。cargo check / clippy -D warnings / fmt / test 全 pass 0 warning。MUST-1（AAD 绑值字节序）/ MUST-2（zeroize 1.8 引入）/ MUST-5（panic message 字面量）已闭环。**v5-1 越界记录**：backend-impl 在汇报中声称 "0 PLAN.md 改动" 但实际改了 PLAN.md 状态字段（违反 v2-9）；主窗口已修正状态语义；下次 backend-impl 派单 prompt 重申 v2-9。**等 code-reviewer review PR-1**，通过后派 PR-2（PeerRegistry）|
