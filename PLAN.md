...
| P2-1.c | 实现阶段第一个 PR（基础设施落地）：把 ADR-009 / ADR-010 / ADR-011 三件套从 trait 签名 + 状态机 + 单测清单 落到 src-tauri/src/peer / app/lifecycle / crypto 三个 module 的实际 Rust 代码 + ≥ 18 条单测（PeerRegistry 7 + Lifecycle 5 + crypto 6） | `IN_PROGRESS (1/3 + 2/3 REVIEW_PASSED, 3/3 待启)` | backend-implementer | **PR-1 REVIEW_PASSED**（commit b3382cb / 18 单测 / APPROVED 0 必修）。**PR-2 PeerRegistry REVIEW_PASSED**（commit 69597a4 + 补丁 commit 待生成 / 累计 32 单测全过 / CHANGES_REQUESTED 2 补丁已落 [中 lock_order_no_deadlock 真单测 + 低 allowed_decision_is_stable 补 3 次断言] / backend-impl 这次诚实粘贴 cargo test --list / git status -s 全真实）。等 PR-3 Lifecycle 启动 |
